...
class PPOAgent:
    ...
    def update(self) -> Tuple[float, float]:
        ...
        # Precompute fixed returns as critic target (GAE return = advantage + value)
        # Using fixed targets prevents the "moving target" problem with TD(0) over multiple epochs
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch in range(num_batches):
                ...
                # Update critic with fixed GAE returns (not live TD targets)
    ...
    def _update_critic(
        ...
        """Update critic with precomputed GAE returns as fixed targets"""
        states_tensor = torch.FloatTensor(states).to(self.device)
